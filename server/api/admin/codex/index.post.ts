...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<{ nom: string; faction: 'imperium' | 'chaos' | 'xenos'; type?: 'armee' | 'soutien' }>(event)
  ...
  await ecrireBrouillon(slug, codexVide(slug, body.nom.trim(), body.faction ?? 'imperium', body.type === 'soutien' ? 'soutien' : 'armee'))
  return { slug }
})
