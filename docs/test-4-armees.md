...
| Périmètre du quota de Commissaires gratuits | La Légion d'Acier le limite aux compagnies. Le Death Korps écrit « par tranche de 1 000 pts dans votre armée, hors appareils de la flotte et soutien titanique », donc une base plus large que les seules compagnies. | Chaque codex suit son propre texte. `par_tranche` n'acceptant qu'une seule section en périmètre, le Death Korps prend l'armée entière, ce qui le rend légèrement permissif sur une liste très chargée en supports. Écart signalé dans sa phrase de budget. |
