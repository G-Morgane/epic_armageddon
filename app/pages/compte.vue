...
/** Les deux faces de la page : ce que le joueur a rangé, puis ce qu'il peut régler. */
const onglets = [
  { id: 'espace', label: 'Mon espace' },
  { id: 'compte', label: 'Mon compte' },
] as const
const onglet = ref<(typeof onglets)[number]['id']>('espace')

/**
 * Nom des codex : une liste ne retient que le slug, et « legion-dacier » sur une
 * pastille ne dit rien au joueur qui cherche sa Légion d'Acier.
 */
const { data: tousCodex } = await useFetch<CodexMeta[]>('/api/codex', { lazy: true, server: false })
...
/** Lien de partage d'une liste, copié d'un clic : il était affiché en texte brut, à recopier à la main. */
const copie = ref('')
async function copierPartage(l: ResumeListe) {
  if (!l.code_partage) return
  try {
    await navigator.clipboard.writeText(`${window.location.origin}/builder/${l.codex}?liste=${l.code_partage}`)
    copie.value = l.id
    setTimeout(() => { if (copie.value === l.id) copie.value = '' }, 2000)
  } catch {
    erreurContenu.value = 'Copie impossible, ouvre la liste et repasse par « Partager »'
  }
}
...
<template>
  <div class="mx-auto max-w-4xl px-4 pb-20 pt-20">
    <h1 class="font-heading text-3xl font-bold text-gold">{{ profile?.display_name || 'Mon compte' }}</h1>
    <p class="mt-2 text-sm text-gray-400">{{ user?.email }}</p>

    <!-- Onglets -->
    <div class="mt-8 flex gap-1 border-b border-gold/10" role="tablist">
      <button
        v-for="o in onglets"
        :key="o.id"
        type="button"
        ...
        class="-mb-px rounded-t-md px-4 py-2 text-sm font-medium"
        :class="onglet === o.id ? 'border border-gold/20 border-b-surface bg-surface text-gold' : 'text-gray-400 hover:text-gray-200'"
        @click="onglet = o.id"
      >
        {{ o.label }}
      </button>
    ...
    <!-- Mon espace : les listes et les favoris -->
    <div v-if="onglet === 'espace'" role="tabpanel">
      <!-- Mes listes d'armée -->
      <section class="mt-8">
        <div class="flex flex-wrap items-baseline justify-between gap-3">
          <h2 class="font-heading text-2xl font-bold text-white">Mes armées</h2>
          <NuxtLink to="/codex" class="text-sm text-gray-400 underline hover:text-gold">Construire une nouvelle armée</NuxtLink>
        </div>
        ...
        <ul v-else class="mt-4 space-y-2">
          <li v-for="l in listes" :key="l.id" class="rounded-lg border border-white/10 bg-surface-light p-3">
            <div class="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              ...
              <span class="rounded bg-white/5 px-1.5 py-0.5 text-[10px] uppercase tracking-wider text-stone-400">{{ nomCodex(l.codex) }}</span>
            ...
            <div class="mt-2 flex flex-wrap items-center gap-2 text-xs">
              ...
              <button
                v-if="l.code_partage"
                type="button"
                class="rounded border border-gold/30 px-2 py-1 text-gold hover:bg-gold/10"
                @click="copierPartage(l)"
              >
                {{ copie === l.id ? 'Lien copié' : 'Copier le lien de partage' }}
              </button>
              <button
                type="button"
