      ...
      <div v-if="ouverte === o.id" class="grid gap-4 bg-gold/5 px-5 pb-5 pt-2 lg:grid-cols-2">
        <div class="space-y-3">
          ...
          <template v-else-if="eff(o).type === 'choix'">
            <div v-for="(p, pi) in eff(o).parmi" :key="pi" class="mb-1 flex flex-wrap gap-2"><input v-model="p.nom" class="champ min-w-40 flex-1" placeholder="Capitaine"><input v-model.number="p.cout" type="number" class="champ w-20"><input v-model="p.categorie" class="champ w-24" placeholder="catégorie"><select :value="p.emplacement ?? ''" class="champ w-28" @change="p.emplacement = (($event.target as HTMLSelectElement).value || undefined) as never"><option value="">emplacement</option><option value="bras">bras</option><option value="carapace">carapace</option></select><input v-model="p.portee" class="champ w-28" placeholder="portée"><input v-model="p.puissance" class="champ min-w-40 flex-1" placeholder="puissance de feu"><button type="button" class="icone hover:text-red-300" @click="eff(o).parmi.splice(pi, 1)">✕</button></div>
            ...
            <p class="text-[11px] text-gray-500">Armes de titan : la catégorie (assaut, tactique, support) et l'emplacement s'affichent entre parenthèses après le nom, et l'emplacement dit sous quelle ligne d'arme le choix apparaît (onglet Unités, « armes au choix »). Emplacement vide : l'arme va sur les deux. Portée et puissance de feu sont facultatives : renseignées, elles suivent le nom dans la liste des armes possibles.</p>
          </template>
