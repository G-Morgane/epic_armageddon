...
/**
 * Pastille des signalements : les bugs encore ouverts, donc les nouveaux et
 * ceux en cours de traitement, comme le filtre « Ouverts » de l'écran. Résolus
 * et refusés sont de l'archive et n'appellent plus rien.
 *
 * Sans cette pastille, l'écran ne se visite que si l'on y pense, et un bug
 * remonté peut dormir.
 */
const signalementsOuverts = ref(0)
onMounted(async () => {
  try {
    const r = await $fetch<{ compteurs: Record<string, number> }>('/api/admin/signalements?compteurs=1')
    signalementsOuverts.value = (r.compteurs.nouveau ?? 0) + (r.compteurs.en_cours ?? 0)
  } catch { /* table absente ou session expirée : pas de pastille */ }
})

const navItems = computed(() => {
  const items: Array<{ label: string; to: string; icon: string; badge?: number }> = [
    ...
    { label: 'Signalements', to: '/admin/signalements', icon: 'bug', badge: signalementsOuverts.value },
  ]
...
function isActive(to: string) {
  // L'écran des armées porte aussi l'édition des codex : sa ligne reste allumée là-bas.
  if (to === '/admin') return route.path === '/admin' || route.path.startsWith('/admin/codex')
    ...
    >
      ...
      <nav class="flex-1 space-y-1 px-3 py-4">
        ...
        >
          ...
          <svg v-else-if="item.icon === 'bug'" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8a4 4 0 014 4v3a4 4 0 01-8 0v-3a4 4 0 014-4zm0 0V6a2.5 2.5 0 015 0M8 12H4m16 0h-4M6.5 7.5L9 9.5m8.5-2L15 9.5M6.5 17.5L9 15.5m8.5 2L15 15.5" /></svg>
          <span class="flex-1">{{ item.label }}</span>
          <span v-if="item.badge" class="rounded-full bg-red-500/20 px-2 py-0.5 text-xs font-semibold text-red-300">{{ item.badge }}</span>
        </NuxtLink>
      </nav>
